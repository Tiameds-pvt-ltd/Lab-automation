...
//        return ApiResponseHelper.successResponse("Samples added to visit successfully", visit);
    ...
    @Transactional
    public ResponseEntity<?> getAllPatientVisitSamples(
        ...
        List<VisitSampleDto> visitSamples = visits.stream()
                .map(visit -> new VisitSampleDto(
                        visit.getVisitId(),
                        visit.getPatient().getFirstName() + " " + visit.getPatient().getLastName(), // patientname
                        visit.getPatient().getGender(),
                        visit.getPatient().getDateOfBirth().toString(), // Convert LocalDate to String
                        visit.getPatient().getPhone(),
                        visit.getPatient().getEmail(), // Corrected order
                        visit.getVisitDate(),
                        visit.getVisitStatus(),
                        visit.getSamples().stream()
                                .map(SampleEntity::getName)
                                .collect(Collectors.toSet()), // Corrected to Set<String>
                        visit.getTests().stream()
                                .map(test -> test.getId())
                                .collect(Collectors.toList()), // List<Long>
                        visit.getPackages().stream()
                                .map(healthPackage -> healthPackage.getId())
                                .collect(Collectors.toList()) // List<Long>
                ))
                .collect(Collectors.toList());

        return ApiResponseHelper.successResponse("All visits with samples", visitSamples);
